...
public class ModelsManagementLink extends ManagementLink implements Saveable, Loadable {
    ...
    @RequirePOST
    public void doConfigSubmit(StaplerRequest2 req, StaplerResponse2 res) throws IOException,
            Descriptor.FormException, ServletException {
        Jenkins.get().checkPermission(getRequiredPermission());
        BulkChange change = new BulkChange(this);
        try {
            this.modelConfigurations = req.bindJSONToList(ModelConfiguration.class, req.getSubmittedForm().get("modelConfigurations"));
            save();
            change.commit();
        } finally {
            change.abort();
